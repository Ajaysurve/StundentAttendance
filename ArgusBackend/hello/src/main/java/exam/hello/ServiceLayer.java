...
public class ServiceLayer {
	...
	@Autowired
	public void setObja(AdminRepo obja) {
		System.out.println("Service : repository Admin DAO  wired to service ");
		this.obja = obja;
	}
	...
	public int doAdminLogin(Admin adminobj) {
		// TODO Auto-generated method stub
		System.out.println(adminobj);
		String email =adminobj.getEmail();
		 Optional<Admin> x = obja.findById(email);
		  ...
		  if(x.isPresent())
			{
				 ...
				 if(admin.getAdminpass().equals(adminobj.getAdminpass()))
				 {
					 return 1;
				 }
				 else
	...
	public int delmsg(int uid) {
		System.out.println("Inside SL");
		System.out.println(uid);
		objFeedRepo.deleteById(uid);
		return 1;
	}
